...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override final def filter(p: Tweet => Boolean): TweetSet = {
    val filteredLeft = left.filter(p)
    val filteredRight = right.filter(p)
    if (p(elem)) new NonEmpty(elem, filteredLeft, filteredRight)
    else filteredLeft.union(filteredRight)
  }
